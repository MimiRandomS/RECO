#!/bin/bash
export LANG=C.UTF-8

show_menu() {
    clear
    echo -e "\e[1;36m╔══════════════════════════════════════════════════╗\e[0m"
    echo -e "\e[1;36m║              🔍 MENÚ DE BÚSQUEDA                 ║\e[0m"
    echo -e "\e[1;36m╠══════════════════════════════════════════════════╣\e[0m"
    echo -e "\e[1;33m║ 1) Buscar archivo por nombre                    ║\e[0m"
    echo -e "\e[1;33m║ 2) Buscar palabra dentro de un archivo         ║\e[0m"
    echo -e "\e[1;33m║ 3) Buscar archivo y luego palabra dentro       ║\e[0m"
    echo -e "\e[1;32m║ 4) Contar líneas de un archivo                 ║\e[0m"
    echo -e "\e[1;32m║ 5) Mostrar primeras N líneas                   ║\e[0m"
    echo -e "\e[1;32m║ 6) Mostrar últimas N líneas                    ║\e[0m"
    echo -e "\e[1;31m║ 7) Salir                                       ║\e[0m"
    echo -e "\e[1;36m╚══════════════════════════════════════════════════╝\e[0m"
    echo -e "Seleccione una opción: "
}

ascci_art_1(){
    clear
    echo "__     __     ______     __         ______     ______     __    __     ______    "
    echo "/\ \  _ \ \   /\  ___\   /\ \       /\  ___\   /\  __ \   /\ -./  \   /\  ___\   "
    echo "\ \ \/ .\ \  \ \  __\   \ \ \____  \ \ \____  \ \ \/\ \  \ \ \-./\ \  \ \  __\   "
    echo "\ \__/ \_\  \ \_____\  \ \_____\  \ \_____\  \ \_____\  \ \_\ \ \_\  \ \_____\ "
    echo "\/_/   \/_/   \/_____/   \/_____/   \/_____/   \/_____/   \/_/  \/_/   \/_____/ "
    echo "-------------------------------------------------------------------------------"
    echo "⠀⠀⠀⠀⠀⠀⠀⢀⡀⠀⠀⠀⣀⡀⢀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⣀⣀⣀⣀⣀⣀⣀⣀⣀⣀⣀⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⣀⡤⠀⠘⢋⣑⣶⣶⣿⣿⣿⣿⣷⣶⣤⣔⣄⠀⠀⠀⠀⠂⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣆⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⢰⡊⠡⠤⠴⣶⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣶⣄⠀⠰⠀⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡄⠈⠓⢦⣀⠀⠙⠻⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣦⡀⢸⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡇⠀⠀⠀⠉⠑⢦⡀⠈⠉⣟⡛⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡟⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡇⠀⠀⠀⠀⠀⠀⠉⠢⣄⠀⠉⢉⣽⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣶⣶⣤⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡇⠀⠀⢀⣄⡀⠀⠀⠀⠈⠑⢄⠀⠳⣭⣽⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡃⠀⢷⣤⡀⠉⠑⠒⠒⠤⣤⣀⡑⡄⠈⢿⣄⣻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⢻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡇⠀⠀⢻⣿⡟⣿⣿⣶⣶⣤⣤⣀⣉⠀⠘⠻⣿⣿⣿⣿⣿⣿⣿⢫⣿⣿⣿⣿⡇⠐⣾⣿⣽⣿⣿⣿⡿⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⡇⠀⢤⠀⠻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⢠⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣟⣽⡄⢈⣽⣿⣿⣿⣿⣿⣦⣌⢻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠇⠀⠈⢧⠀⠸⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⣿⣿⢸⡘⡄⠻⣽⣿⣿⣿⣿⣽⣶⣽⢝⢿⣿⣿⣿⣿⣿⣿⣿⣿⣦⣤⣤⣤⣤⣄⣀⡀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠈⠳⡀⠈⠻⣿⣿⣿⣿⣿⣿⣿⣿⡇⣿⣿⣿⣿⣿⢿⣧⣽⣿⡇⢻⣿⢿⣧⠘⢦⡛⢿⣿⣿⣿⣿⣛⠛⢷⣦⣙⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣶⣦⡄"
    echo "⠀⠀⠀⠀⠀⠹⢦⡀⠈⢿⣿⣿⣿⣿⣿⣿⣷⡸⢿⣿⣿⣿⣿⣿⠿⣿⣿⣿⣿⡀⢹⢷⣄⠈⢳⣌⣻⣿⣿⣿⣷⠂⠈⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠇"
    echo "⠀⠀⠀⠀⠀⠀⠀⠉⠲⢄⡻⢿⣿⣿⣿⣿⣿⣿⣶⡿⢿⣿⣯⣘⡄⠘⢧⠈⠙⠿⣦⣇⠙⢆⢀⣘⣶⣾⣟⣿⣿⣧⣤⣼⣿⣿⣿⡿⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠛⠁⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠢⣉⠛⢿⣿⣿⡿⢿⣿⣇⠻⢿⣍⣧⣀⣈⣀⣴⣦⡈⢻⣶⢏⣿⣧⣴⡾⢿⡿⢿⣟⠛⠛⠿⣿⣿⣧⢿⣿⣿⣿⣿⣿⣿⣿⡿⠟⠁⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠑⢈⢿⣿⢸⣿⣽⣿⣧⠀⠚⠶⠾⠶⠶⠾⣿⣭⣉⣠⠈⢉⣹⣷⣶⣾⣿⣷⢬⣤⣠⣾⣿⣿⣎⣿⣿⣿⣿⣿⣿⣿⣋⣤⣤⣤⣤⣤⣀⣠⡄"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣶⣦⣭⣿⣿⢸⣿⣻⡿⠈⠙⠀⠀⠀⠀⠀⠀⠀⠙⠟⠀⣀⣿⣄⠛⠋⠉⠉⠉⠉⢻⣿⣿⣿⢍⣿⣿⣯⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡇"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠘⢻⣿⣿⣿⣧⢸⣷⠛⢹⡇⠀⠀⠀⠀⠀⠀⣀⡀⠀⠀⠼⣿⡿⠟⠁⠀⣤⢦⡄⠀⢠⣿⣿⣿⣟⣯⣿⣿⡏⣹⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠃"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠜⢻⣿⣿⣧⡻⢿⣯⣿⠀⠀⠀⠀⠀⠀⠹⢿⢶⡶⠶⣶⣶⡶⢒⣋⡽⣾⠁⠀⣼⣿⣿⡟⣿⣧⣼⠟⢹⣿⣿⣿⣿⣿⣿⣿⣿⡿⠋⠁⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠙⢓⣿⣦⣀⣹⡄⠀⠀⠀⠀⠀⠀⠈⠳⠷⠶⠶⠤⠴⠾⠋⣰⠟⠀⣼⣿⣿⡏⣽⣛⠳⠋⣰⣿⣿⣿⣿⣿⣿⣿⠟⠉⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣠⣿⣿⣿⣿⣧⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⣤⣀⡀⠀⠀⠀⠀⣠⣿⣿⡟⣽⢋⣽⣿⣿⣿⡿⠿⠟⠛⠉⠁⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠴⠛⠉⠛⠛⠛⠛⢻⠻⣄⠀⠀⠀⠀⠀⠀⠋⠀⠙⢻⣿⡆⠀⠀⣠⣾⣿⣿⣴⣿⣿⣿⣿⣿⣿⣿⣿⣶⣤⣄⣀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢸⠀⢏⠷⣄⠀⠀⠀⠀⠀⠀⠀⠈⠹⠇⠀⢀⣾⣿⣿⣿⣿⡟⠛⠛⣿⣿⡿⠿⠿⠿⠿⠿⠿⠿⠶⠄⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠸⠀⠸⣆⠘⢳⣄⡀⠀⠀⠀⠀⠀⢀⣤⣾⣿⣿⣿⣿⢿⣿⣇⠀⠻⢿⣿⠇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡞⠀⠀⠹⡄⠀⠙⠿⢤⣄⣀⣴⣾⣿⣿⣿⣿⠛⡻⠃⢸⣿⣿⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣇⣀⣀⣀⣹⡄⠀⠀⠀⠀⠉⠛⢋⣹⣿⣿⣛⣾⣥⣶⣿⣿⣿⣄⠀⠀⢀⣐⣆⢀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⡤⣤⣀⡀⠀⣠⠞⠁⣈⠛⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡆⠻⣦⣄⠀⠀⢀⡀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡀⣴⣿⣿⣿⣿⣿⣿⣿⠴⠿⠛⠛⣏⠛⣿⣿⣿⡟⣿⣿⣿⣿⣿⡟⠁⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠇⠠⣝⣿⣷⣶⡆⣧⣀⣀⣀⣠⡄"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⣶⣶⠖⠀⢵⣿⣿⣿⣿⡿⠛⠋⠁⠀⣠⣤⣾⣿⣦⡀⢽⣿⣿⣿⣿⣿⣿⢏⣰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡟⢀⣹⣿⣿⣿⣿⣱⡿⢛⣿⣿⣿⡇"
    echo "⠀⠀⠀⠠⠀⣠⢟⣡⣿⣿⣃⣺⣿⣿⣿⣿⡇⢀⣤⣴⣶⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡟⣠⣽⣿⣿⣿⣿⣧⡿⣣⣿⣿⣿⣿⡇"
    echo "⠀⠀⠀⢀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    sleep 3
    clear
}

ascci_art_2(){
    clear
    echo "          ______     __  __     ______    "
    echo "          /\  == \   /\ \_\ \   /\  ___\   "
    echo "          \ \  __<   \ \____ \  \ \  __\   "
    echo "          \ \_____\  \/\_____\  \ \_____\ "
    echo "          \/_____/   \/_____/   \/_____/ "
    echo "--------------------------------------------------------------------"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢰⢆⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢸⢠⠳⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⠀⠀⢸⢸⢳⡙⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⠖⡏⠀⠀⠀⢸⠀⠐⡜⣆⠀⠀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣠⢞⠵⢸⠀⠀⢀⡇⣸⠀⡆⠘⣌⢆⠀⣷⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢠⢞⡵⠁⡆⡇⠀⡠⠋⡼⠀⠀⡇⠀⠘⠈⢧⡏⡄⢠⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⢠⠀⠀⠀⠀⢀⡴⣡⡯⠀⢀⡇⣧⠞⠁⡰⠃⠀⠀⣧⠀⠀⠀⢸⡇⢃⢸⢇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⢸⡀⠀⠀⢠⢎⡜⡿⠁⠀⢸⣇⡵⠁⠀⠀⠀⠀⠀⣿⠀⠀⠀⠈⠀⢸⣸⠘⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⢸⢣⠀⡴⣡⣿⠁⠃⠀⢀⣾⡿⠁⠀⠀⠀⠀⠀⠀⣿⠀⠀⠀⠀⠀⠈⡏⠀⢇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⢸⠈⢇⡇⣿⡏⠀⠀⠀⣼⣿⠃⠀⠀⠀⠀⢀⠇⡰⣿⠀⠀⠀⠀⠀⡇⠁⠀⢸⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠸⠐⠄⠀⠏⡇⠀⠀⣧⣿⡇⡀⡜⢰⠀⠀⡘⡐⠁⠏⡆⠀⠀⡄⢠⡇⡄⠀⠈⡆⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠈⠦⢠⣧⠀⣆⣿⣿⢁⣷⣇⡇⠀⣴⣯⠀⠀⠀⡇⠀⣸⡇⣾⡿⠁⠀⡀⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⢀⠀⠀⠀⢀⢀⢠⠀⠸⣿⣆⢹⣿⣿⣾⣿⣿⣠⢾⠛⠁⠀⠀⠀⡇⡠⡟⣿⣿⠃⠀⠀⣿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠘⡶⣄⠀⢸⠸⣼⣧⡀⣿⣿⣾⣿⣿⣿⣿⣿⡇⠘⠀⡀⠀⠀⢠⠟⠀⠃⢹⣥⠃⠀⢠⢏⣜⡄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠙⡌⠳⢄⣣⠹⣿⣿⣿⣿⣿⣿⣿⡿⢿⣿⡇⠀⠀⢀⣄⣴⡢⠀⠀⠀⡿⣯⠀⠐⠁⠘⣻⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠘⢎⢶⣍⣀⠈⢿⣿⣿⣿⣿⣿⣿⣦⠑⣤⡀⠀⣰⠟⡿⠁⠀⠀⠈⠀⠁⠀⠀⡀⡰⠃⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠈⢣⣻⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⡀⠘⣷⣾⣿⡆⠀⠀⠀⠀⠀⠀⠀⠀⠀⡵⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠑⣝⠻⠿⣿⣿⣿⣿⣿⣿⣿⣇⠀⣿⣿⣿⣇⣀⣤⠆⠀⠁⠀⠉⠀⠸⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠈⠉⡇⢸⣿⣿⣿⣿⣿⣿⣿⣼⣿⣿⣿⣿⣿⠋⠀⠀⠀⠀⠀⠐⢤⡀⠙⢦⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠱⢬⣙⠛⠿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣏⡄⠀⠀⠀⠀⠀⠀⠈⠻⠆⠀⠈⠑⠒⣿⣦⣆⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠑⠲⣼⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⣀⣀⣀⠀⠀⣀⣀⣠⣴⣾⣾⣿⣿⣿⣿⣿⣷⣦⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣘⣿⣷⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣦⠤⣀⣤⣀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⢘⣿⠟⣡⣶⣶⣤⡄⠙⣿⣿⣿⣿⣿⣿⣟⡛⠿⠿⣿⣿⣿⣿⣿⣿⣿⡿⠿⢿⣿⣿⣿⡿⠟⣩⣿⣿⣿⣿⡀⠀⠀⢏⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⣶⣿⢋⣼⢿⠿⢛⣿⠷⢶⣶⠂⠿⣿⣿⣿⣿⣿⣷⣶⣤⣀⡀⠉⠉⠀⠀⣀⣀⡀⠀⠀⠀⠠⢾⣿⣿⣿⣿⣿⠇⠀⠀⣘⠢⡀⠀⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⢀⣽⠁⠘⠁⠀⠀⠁⠀⠠⠟⠛⣿⣄⡩⢉⣿⣿⣿⣿⣿⡿⠋⠀⡠⣶⣶⣶⡶⣶⣶⣾⠿⠶⠀⠀⠻⣿⣿⣿⣿⠀⠀⠠⣿⣷⡘⢆⠀⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⡸⠀⠀⠀⠀⠀⠀⠀⠈⠀⠀⣿⣿⣤⡈⣿⣿⣿⣿⡟⠁⣠⣾⡇⠀⣿⣿⣆⠀⠀⠀⠀⣀⣠⣆⠀⢹⣿⣿⡿⠀⠀⢠⣿⣿⣿⡘⡆⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⡇⠀⠀⠀⠀⠀⠀⠀⠀⠀⢰⣿⣿⠁⣺⣿⣿⣿⡿⠀⢰⡟⠛⠇⠘⠿⣿⣇⠀⠀⣀⠀⢀⣽⣿⡀⠀⣿⣿⠃⠀⠀⢸⣿⣿⣿⣧⢸⠀⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⢣⠀⠀⠀⠀⠀⠀⠀⢀⣴⠟⣿⠃⢰⠨⣿⣿⣿⠃⠀⣿⡇⢀⠀⢰⠀⠛⠛⠀⠀⠛⠀⠈⠉⠹⠇⠀⣿⡏⠀⠀⠀⣹⣿⣿⣿⣿⠘⢦⠀⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠈⡆⠀⠀⠀⠀⣠⡶⠋⠁⡼⠃⠀⣾⠃⣿⣿⣿⠀⠀⡟⢀⡜⠀⢋⣠⣶⠀⠀⠒⠒⠀⠀⣶⡾⠀⢠⠏⠀⠀⣠⣾⣿⣿⣿⣿⢿⠁⠀⣣⠀⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⣼⣷⢤⣤⢶⠟⠁⠀⠀⠀⠀⢀⣼⡏⣸⣿⣿⣿⣇⠀⢻⣿⡇⠀⣿⣿⣿⠀⠀⣶⣶⠀⢸⣿⠃⠀⠎⠀⠀⠀⠿⣿⣿⣿⣿⡿⠀⠀⢀⣯⢇⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⢰⣿⣿⠘⠁⠁⠀⠀⢀⣠⣴⣾⣿⠟⣰⣿⣿⣿⣿⣿⡄⠀⠻⠀⣠⣿⣿⣿⠀⠀⠉⠉⠀⠘⠁⢀⠌⠀⠀⠀⢀⠀⠀⠈⠉⠀⠀⠀⢀⣾⣿⡿⠀⠀"
    echo "⠀⠀⠀⠀⠀⠀⡟⣿⡏⠀⠀⢀⣠⣾⣿⣿⡿⠛⣡⠀⣿⣿⣿⣿⣿⣿⣿⣦⣀⠈⠙⠻⠿⠿⠶⠾⠟⠃⠀⢀⠔⠁⠀⠀⠀⠀⣾⣆⠀⠀⠀⠀⣰⢀⣾⣿⣿⣧⡇⠀"
    echo "⠀⠀⠀⠀⠀⢸⠁⣿⠃⢀⣴⣿⣿⣿⠟⢻⢁⣾⣿⢀⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣦⣤⣄⣀⡀⠠⠤⠐⠊⠀⠀⠀⠀⠀⢀⡰⣿⣿⡆⠀⠀⣿⣧⣿⣿⣿⣿⣿⡇⠀"
    echo "⠀⠀⠀⠀⠀⣌⠀⠘⢠⣿⣿⣿⡟⠁⢀⣏⣾⣿⡇⣸⣿⣿⣿⣿⣿⣟⠛⠛⠛⠛⠛⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⡰⠋⢱⣿⣿⣧⠀⢠⣿⣿⣿⣿⣿⣿⡏⢱⠀"
    echo "⠀⠀⠀⠀⠀⠈⠑⠢⢿⣿⣿⡟⠀⢀⣾⣿⣿⡟⢠⣿⣿⣿⣿⡿⠿⢿⣿⣷⣶⣤⣤⣤⣄⣤⣤⣤⠤⠖⠂⠀⠀⣠⠊⠀⠀⠀⢿⣿⣿⠀⢸⣿⣿⣿⣿⣿⣿⠇⢸⠀"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠙⠣⢤⣈⣿⣿⠏⣰⣿⣿⣿⣷⣭⣍⣑⠂⠀⠀⠈⠉⠉⠉⠉⠉⠀⠀⠀⠀⠀⢀⡼⠁⠀⠀⠀⠀⠈⢿⢿⠀⣼⣿⣿⣿⣿⣿⣧⢴⣾⡄"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠑⠚⠿⠿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠶⠶⠶⠖⠒⠂⠀⠀⠀⠀⢠⠞⠀⠀⠀⠀⠀⠀⠀⠘⡄⠀⣿⣿⣿⣿⣿⣿⣡⣿⣿⡇"
    echo "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠉⠉⠉⠓⠒⠒⠒⠒⠒⠒⠒⠒⠊⠉⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠱⠤⠛⠛⠿⠯⠭⠭⠙⠒⠚⠁"
    sleep 3
}

logic() {
    ascci_art_1
    while true; do
        show_menu
        read -r option
        clear
        case $option in
            1) search_file ;;
            2) search_word_in_file ;;
            3) search_file_and_word ;;
            4) count_lines ;;
            5) show_head_lines ;;
            6) show_tail_lines ;;
            7) func_exit ;;
            *) echo -e "Opción no válida." ;;
        esac
        echo -e "\nPresione Enter para continuar..."
        read -r
    done
}

search_file() {
    echo -e "Ingrese la ruta donde desea buscar (o presione Enter para usar la ruta actual): "
    read BASE_DIR
    if [ -z "$BASE_DIR" ]; then
        BASE_DIR=$(pwd)
    fi

    echo -e "Ingrese el nombre del archivo a buscar: "
    read filename

    results=$(find "$BASE_DIR" -type f -iname "*$filename*")
    count=$(echo "$results" | wc -l)

    if [ "$count" -gt 0 ]; then
        echo -e "Archivos encontrados ($count):"
        echo "$results"
        sleep 3
    else
        echo -e "No se encontraron archivos."
        sleep 1
    fi
}


search_word_in_file() {
    echo -e "Ingrese ruta del archivo donde buscar: "
    read file
    if [ -f "$file" ]; then
        echo -e "Ingrese la palabra a buscar: "
        read word
        grep -n --color=auto "$word" "$file"
        count=$(grep -o "$word" "$file" | wc -l)
        echo -e "Total apariciones: $count"
        sleep 3
    else
        echo -e "El archivo no existe."
        sleep 1
    fi
}

search_file_and_word() {
    search_file
    echo -e "Seleccione un archivo de la lista anterior: "
    read file
    if [ -f "$file" ]; then
        search_word_in_file "$file"
        sleep 3
    else
        echo -e "El archivo no existe."
        sleep 1
    fi
}

count_lines() {
    echo -ne "Ingrese ruta del archivo: "
    read file
    if [ -f "$file" ]; then
        lines=$(wc -l < "$file")
        echo -e "Total de líneas: $lines"
        sleep 3
    else
        echo -e "El archivo no existe."
        sleep 1
    fi
}

show_head_lines() {
    echo -e "Ingrese ruta del archivo: "
    read file
    if [ -f "$file" ]; then
        echo -ne "Ingrese el número de líneas a mostrar: "
        read n
        head -n "$n" "$file"
        sleep 3
    else
        echo -e "El archivo no existe."
        sleep 1
    fi
}

show_tail_lines() {
    echo -ne "Ingrese ruta del archivo: "
    read file
    if [ -f "$file" ]; then
        echo -ne "Ingrese el número de líneas a mostrar: "
        read n
        tail -n "$n" "$file"
        sleep 3
    else
        echo -e "El archivo no existe."
        sleep 1
    fi
}

func_exit(){
    ascci_art_2
    echo -e "Saliendo... "
    exit 0
}

logic
